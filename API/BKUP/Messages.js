...
//show open batches	
	...
	function retrRecsOB(rows)
	{
		...
		for(key in rows)
		{
			...
			BTYPE = encodeURIComponent(BTYPE);
			PNID =encodeURIComponent(PNID);
		    aRecs[rescount]=({REC: rescount, BID: BID, VERGUID: VERGUID, BTYPE : BTYPE, SSID: SSID, CREATE: CDT, PNID: PNID});
		}
...
/* upgrade this to get picking batches also */
	/* return the batch type also */
		console.log('Will attempt to display VERIFY & PICK batches for the current user');
		let token    = req.body.token;
		let tokenObj=lauth.testToken(token);
		if(typeof tokenObj=='undefined'){res.sendStatus(403);return;}	
	    let user=tokenObj.user;
	let userid = tokenObj.guid;
	if (user.match('@')) { userw = user.split('@'); user = userw[0]; }
	console.log('Tested the token, guid is : ' + userid + ' and the user is : ' + user + ' : ');
	//no change to the query as it's not filtered
		 const q2='SELECT * FROM s_v_dat_batch_hdr WHERE STATUS= ? and ((VERGUID = ?) or (PACKGUID = ?))';
		 console.log('Running query: ' + q2);
	connection.query(q2, ['OPEN',userid,userid], (error, rows) => {
         if (error) {
		 res.json({status: 3, token: '', batch: '', guid: 0 , uLvl: -1});
	...
	function retrRecsOBUser(rows,m)
	{
		...
		for(key in rows)
		{
			...
			BTYPE = encodeURIComponent(rec.BTYPE);
			STYPE = encodeURIComponent(rec.STYPE);
			VERGUID = encodeURIComponent(rec.VERGUID);
			PACKGUID = encodeURIComponent(rec.PACKGUID);
		    ...
		    aRecs[rescount]=({REC: rescount, BID: BID, BTYPE: BTYPE, STYPE: STYPE, VERGUID: VERGUID, PACKGUID: PACKGUID, SSID: SSID, CREATE: CREATE, PNID: PNID, CUSTNAME: CUSTNAME});
		}
		console.log(aRecs);
		rChain=JSON.stringify(aRecs);
		return(rChain);
...
//
// for getting product name AND writing the count line
	function commitProductLookup(req, res, connection)
	{
		...
		prodName=pns[0];
		PID=pns[1];
		if(prodName==null){prodName='Error 1 in Commit Retrieval';}
		if(typeof prodName=='undefined'){prodName='Error 2 in Commit Retrieval';}
	//	console.log('ProdName/PID for Commit Returned: ' + prodName + ' ' + pid  );
		let BID=req.body.batchID;
		console.log('Now attempting to commit line with BID: ' + BID);
		
		writeBatchLine(req, res, connection, BID, SCANID, CLID, PID, prodName);
		return;
		});						
	}
	...
	function procRetrHead(rows)
	{
		...
		for(key in rows)
		{
			...
			CUSTGUID=rec.CUSTGUID;
			PICKERNAME=rec.PICKERNAME;
			PICKEDDATE=rec.PICKEDDATE;
			if(typeof(VERGUID)=='undefined'){VERGUID=0;}
			if(typeof(SSID)=='undefined'){SSID=0;}
			if(typeof(CREATE)=='undefined'){CREATE=0;}
			if(typeof(PNID)=='undefined'){PNID=0;}
			if (typeof (CUSTNAME) == 'undefined') { CUSTNAME = ''; }
			if (typeof (BTYPE) == 'undefined') { BTYPE = ''; }
		    if(typeof(CUSTGUID)=='undefined'){CUSTGUID='';}	
			if(typeof(PICKERNAME)=='undefined'){PICKERNAME='';}	
			VERGUID=encodeURIComponent(VERGUID);
		    SSID=encodeURIComponent(SSID);
		    CREATE=encodeURIComponent(CREATE);
		    BID =encodeURIComponent(BID);
			...
			CUSTGUID=encodeURIComponent(CUSTGUID);	
			PICKERNAME=encodeURIComponent(PICKERNAME);
			PICKEDDATE=encodeURIComponent(PICKEDDATE);
outRec=({BID: BID, VERGUID: VERGUID, SSID: SSID, CREATE: CDT, PNID: PNID, BTYPE: BTYPE, CUSTNAME: CUSTNAME, CUSTGUID: CUSTGUID, PICKERNAME: PICKERNAME, PICKEDDATE: PICKEDDATE});
		}
...
// request a new batch
	...
	function openANewBatch(req, res, connection)
	{
		...
		if(typeof cName=='undefined'){res.json({status: 1,  batchID: 0, PNAME: ''});return}
		if(cName==null){res.json({status: 1,  batchID: 0, PNAME: ''});return}
		let cGUID=cName;
		let cArr=cGUID.split("GUID:");
		if(typeof cArr[1]=='undefined'){cGuid=0;}
		if(cArr[1]== null){cGuid=0;}
		else{cGuid=cArr[1];}
		console.log('Right into query creation for batch create with CNAME = ' + cName);
	 const q2='INSERT INTO s_v_dat_batch_hdr (STATUS, VERGUID, PNID, PICKERNAME, CUSTNAME, CUSTGUID) values (?, ?, ?, ?, ?, ?)';
	 connection.query(q2, ['OPEN',userID, PNID, packerName, cName, cGuid], (error, result) => {
		if(error) throw error;
        //check success
		console.log('Trying to create the new batch header, cname was: ' + cName);
		console.log(result);
...
function finishBatch1(req, res, connection)
	...
	//	console.log('Trying to Finish and Recon a Batch. Delete any existing lines');
		...
		/* clear out the summary lines in the batch summary */
		/* probably moot */
    q=  'delete from s_v_dat_batch_sum where PNID=? and BID=?';
	console.log('in FB1 BID is: ' + BID);
...
//		console.log('Trying to Sum PackNote Details');
	    ...
	    let user=tokenObj.user;
		let userID=tokenObj.guid;
		let batchID=req.body.BID;
		let PNID=req.body.PNID;
		/* create the original packing note summary*/
		/* this seems to be working fine */
    q=  'INSERT into s_v_dat_batch_sum (PNID, BID, PID, LINETYPE, PQTY) ';
q = q + 'select ?, ?, pid, \'PACK\', sum(QTYSUP) from s_sup_dat_d where DOCID=? and (LINETYPE=\'SUPPLYITEM\' or LINETYPE=\'KITSUPPLYITEM\') and ';
...
async function finishBatch3(req, res, connection)
	{
		...
		/* batchid is not being written properly */
...
q = q + 'group by PID order by PID		';
	console.log(q);
	console.log('FB3: PNID: ' + PNID + ' BID: ' + BID);
		 connection.query(q, [PNID, BID, BID], (error, result) => {
		if(error) throw error;
        //check success
	//	console.log(result);
...
//	console.log('PNID: ' + PNID + ' BID: ' + batchID);	
        ...
        //check success
		//console.log(result);
		finishBatch4_1(req, res, connection);		
	 });	
		return;
	}
// inject a process here to do the product names and barcodes
async function finishBatch4_1(req, res, connection)
	{
		console.log('Trying to Fix Names in 4_1');
		let token    = req.body.token;
		let tokenObj=lauth.testToken(token);
		if(typeof tokenObj=='undefined'){res.sendStatus(403);return;}	
	    let user=tokenObj.user;
...
q = 'UPDATE s_v_dat_batch_sum AS a1, p_dat_prod AS a2  SET a1.PRODNAME= left(a2.name,50) ';
q=q+'where a2.ID = a1.PID AND a1.PNID=? AND a1.BID=? and (a1.LINETYPE=\'PACK\' or a1.LINETYPE=\'SCAN\')';
...
async function finishBatch5(req, res, connection)
	{
		...
		console.log('Trying to Clean Redundant Lines in FB5');
				console.log('Trying to Clean Redundant Lines in FB5');
						console.log('Trying to Clean Redundant Lines in FB5');
		let token    = req.body.token;
		let tokenObj=lauth.testToken(token);
		...
		let BID=req.body.BID;
		let PNID=req.body.PNID;
		if(BID == null){console.log('WHOA = dangerous query at finishbatch5. BID is null');}
		if(BID == 'undefined'){console.log('WHOA = dangerous query at finishbatch5. BID is undefined');}
		if(PNID == null){console.log('WHOA = dangerous query at finishbatch5. PNID is null');}
		if(PNID == 'undefined'){console.log('WHOA = dangerous query at finishbatch5. PNID is undefined');}	
		console.log('Trying to Clean Redundant Lines in FB5, BID = ' + BID);		
...
q=q+'s_v_dat_batch_sum as a2 WHERE a1.PNID=? and a1.PID=a2.PID and ';
q=q+'a2.BID=? and a2.LINETYPE=\'SCAN\' GROUP BY a1.PID HAVING flip>1) c )'
	//console.log(q);
		 connection.query(q, [PNID, BID], (error, result) => {
		if(error) throw error;
        //check success
...
//	console.log(q);
	...
	console.log('In the query scope BID is ' + BID);	
		let aRecs=new Object();
		let rescount=0;
	    //console.log(rows);
		for(key in result)
	...
	//		console.log('Retrieved BID, PID, etc: ' + BID + ' ' + PID);
			...
			PRODNAME =encodeURIComponent(PRODNAME);			
		    aRecs[rescount]=({REC: rescount, ID: ID, BID: BID, PNID: PNID, PID: PID, LINETYPE: LINETYPE, SQTY: SQTY, PQTY: PQTY, PRODNAME: PRODNAME});
		}
		//console.log(aRecs);
		console.log('After the query scope BID is ' + BID);		
		rChain=JSON.stringify(aRecs);
	//	console.log('Scope? Rchain set to: ' + rChain);
		...
		q = 'update s_v_dat_batch_hdr set status=\'RECONCILING\' where BID= ?';
				 connection.query(q, [BID], (error, result) => {
				  ...
				  console.log('Should have bloody closed here - batch id: ' + BID);
	 });	 
		return;
	}
