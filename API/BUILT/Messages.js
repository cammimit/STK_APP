...
function showOpenBatchesUser(req, res, connection, m) {
    ...
    var q2 = 'SELECT * FROM s_v_dat_batch_hdr WHERE STATUS= ? and ((VERGUID = ?) or (PACKGUID = ?))';
    console.log('Running query: ' + q2);
    connection.query(q2, ['OPEN', userid, userid], function (error, rows) {
        if (error) {
            res.json({ status: 3, token: '', batch: '', guid: 0, uLvl: -1 });
...
function commitProductLookup(req, res, connection) {
    ...
    connection.query(q2, [SCANID], function (error, rows) {
        ...
        PID = pns[1];
        if (prodName == null) {
            prodName = 'Error 1 in Commit Retrieval';
        }
        if (typeof prodName == 'undefined') {
            prodName = 'Error 2 in Commit Retrieval';
        }
...
function openANewBatch(req, res, connection) {
    ...
    var cGUID = cName;
    var cArr = cGUID.split("GUID:");
    if (typeof cArr[1] == 'undefined') {
        cGuid = 0;
    }
    if (cArr[1] == null) {
        cGuid = 0;
    ...
    var q2 = 'INSERT INTO s_v_dat_batch_hdr (STATUS, VERGUID, PNID, PICKERNAME, CUSTNAME, CUSTGUID) values (?, ?, ?, ?, ?, ?)';
    connection.query(q2, ['OPEN', userID, PNID, packerName, cName, cGuid], function (error, result) {
        if (error)
            throw error;
        //check success
...
function finishBatch3(req, res, connection) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            connection.query(q, [PNID, BID, BID], function (error, result) {
                if (error)
                    throw error;
                //check success
                //	console.log(result);
...
function finishBatch4_1(req, res, connection) {
    return __awaiter(this, void 0, void 0, function () {
        var token, tokenObj, user, userID, batchID, PNID;
        return __generator(this, function (_a) {
...
function finishBatch5(req, res, connection) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            PNID = req.body.PNID;
            if (BID == null) {
                console.log('WHOA = dangerous query at finishbatch5. BID is null');
            }
            if (BID == 'undefined') {
                console.log('WHOA = dangerous query at finishbatch5. BID is undefined');
            }
            if (PNID == null) {
                console.log('WHOA = dangerous query at finishbatch5. PNID is null');
            }
            if (PNID == 'undefined') {
                console.log('WHOA = dangerous query at finishbatch5. PNID is undefined');
            }
