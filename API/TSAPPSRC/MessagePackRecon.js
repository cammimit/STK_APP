...
//
	...
	function procProdNameCommit(rows, res, CLID)
	{
		...
		if(typeof prodName=='undefined'){res.json({status: 6, CCLN: CLID});}
		if(prodName.length<1){res.json({status: 6, CCLN: CLID});}
		if(typeof pid=='undefined'){res.json({status: 7, CCLN: CLID});}
		if(pid.length<1){res.json({status: 7, CCLN: CLID});}
	return prodName+'|'+pid;
	}
...
// request a new batch
	...
	function retrRecsOBUserNew(rows, req, res, connection)
	{
		...
		if(rescount>0){res.json({status: 6, BID: BID});return;}
		else{openANewBatch(req, res, connection); return;}
	}
	function openANewBatch(req, res, connection)
	{
	 ...
	 const q2='INSERT INTO s_v_dat_batch_hdr (STATUS, VERGUID,PNID) values (?, ?, ?)';
	 connection.query(q2, ['OPEN',userID, PNID], (error, result) => {
		if(error) throw error;
        //check success
		console.log(result);
		...
		if(insertId>0){res.json({status: 0, batchID: insertId} );return}
		else{res.json({status: 1,  batchID: 0});return}
	 });
	}
